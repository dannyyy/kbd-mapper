...
export function computeLegendPlacements(
  ...
  if (baseBinding && baseBinding.type !== 'none') {
    ...
    if (baseBinding.holdLabel && (baseBinding.type === 'mod-tap' || baseBinding.type === 'layer-tap')) {
      placements.push({
  ...
  const cornerPositions: Array<{ x: number; y: number; anchor: 'start' | 'end' }> = [
    ...
    { x: keyWidth - pad, y: pad + theme.typography.secondaryLabelSize, anchor: 'end' },   // top-right (L4)
  ]
