...
export function useExport() {
  ...
  function prepareExportSvg(): SVGElement | null {
    ...
    const expandedLayers = renderer.querySelectorAll('.expanded-layer')
    let totalHeight = 0
    let maxWidth = 0
    const padding = 20
    ...
    svgs.forEach((svg, i) => {
      ...
      if (layer) {
        ...
        headerGroup.setAttribute('transform', `translate(${padding}, ${yOffset})`)
        ...
        text.setAttribute('font-family', themeStore.currentTheme.typography.fontFamily)
        text.setAttribute('fill', themeStore.currentTheme.colors.legendColors.primary)
  ...
  function cleanSvg(svg: SVGElement) {
    ...
    svg.querySelectorAll('.keycap.selected rect').forEach(rect => {
  ...
  async function exportSvg(): Promise<void> {
    ...
    downloadText(svgString, `${projectStore.project.name}.svg`, 'image/svg+xml')
  }
  ...
  async function exportPdf(): Promise<void> {
    ...
    try {
      await svg2pdf(svg, pdf, { x: 0, y: 0, width: svgWidth, height: svgHeight })
  ...
  async function exportAs(format: 'svg' | 'png' | 'pdf', scale = 2): Promise<void> {
    switch (format) {
      case 'svg': return exportSvg()
      case 'png': return exportPng(scale)
